...
class Configuration:
    def __init__(self):
        ...
        # mask
        parser.add_argument('--mask_ratio', default=0.0, type=float, help='mask ratio for non-center area')
        parser.add_argument('--keep_center', action='store_true', help='keep center area')
        ...
        parser.add_argument('--data_folder', default=r'/root/autodl-tmp/University-Release', type=str)
...
if __name__ == '__main__':
    ...
    if config.name:
        model_path = "{}/{}/{}".format(config.model_path, config.model, config.name)
    else:
        model_path = "{}/{}/{}".format(config.model_path, config.model, time.strftime("%m%d%H%M%S"))
    ...
    train_dataset = U1652DatasetTrain(query_folder=config.query_folder_train,
                                      ...
                                      mask_ratio=config.mask_ratio,  # 新增
                                      patch_size=config.patch_size,  # 新增（如果添加了该参数）
                                      keep_center=config.keep_center,  # 新增
                                      center_ratio=config.center_ratio  # 新增
                                      )
